...
from .schema import ZomatoInvoice
...
# Column widths for Line Items sheet
LINE_ITEM_COLUMNS = [
...
def write_excel(invoice: ZomatoInvoice, output_path: str) -> str:
    """
    ...
    Args:
        invoice: ZomatoInvoice object with all extracted data.
    ...
    ws_summary.sheet_properties.tabColor = "4472C4"  # Blue

    _write_summary_sheet(ws_summary, invoice)

    # --- Sheet 2: Line Items ---
    ws_items = wb.create_sheet(title="Line Items")
    ws_items.sheet_properties.tabColor = "70AD47"  # Green

    _write_line_items_sheet(ws_items, invoice)
...
def _write_summary_sheet(ws, invoice: ZomatoInvoice) -> None:
...
def _write_line_items_sheet(ws, invoice: ZomatoInvoice) -> None:
    """Write the Line Items sheet (tabular layout)."""
    # Write headers
    for col_idx, col_config in enumerate(LINE_ITEM_COLUMNS, start=1):
    ...
    last_col_letter = get_column_letter(len(LINE_ITEM_COLUMNS))
